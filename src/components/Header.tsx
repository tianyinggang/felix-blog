...
const Header: React.FC = () => {
  ...
  return (
    <header className="md:flex">
      <div
        ref={mainRef}
        className={`main-nav lower-glassmorphism bg-backgroundLight dark:bg-backgroundDark z-30 top-0 shadow-sm fixed duration-400 px-4 sm:px-8 h-16 w-full ${navClassList.join(' ')}`}
      >
        <div className="w-full h-full mx-auto max-w-6xl flex items-center justify-between">
          <Link
            href="/"
            className="text-xl sm:text-2xl md:hover:text-brandPrimary dark:md:hover:text-brandAccentLight focus-visible:outline-brandPrimary dark:focus-visible:outline-brandAccentLight"
          >
            SatNaing
            <span className="text-brandPrimary dark:text-brandAccentLight">.dev</span>
          </Link>
          <nav className="flex items-center">
            <div className="glassmorphism md:bg-transparent md:dark:bg-transparent md:backdrop-blur-none fixed md:static bottom-4 z-30 left-1/2 md:left-auto transform -translate-x-1/2 md:transform-none bg-backgroundLight dark:bg-carddark dark:text-textlight w-11/12 rounded drop-shadow-lg md:drop-shadow-none">
              <ul className="flex justify-evenly items-center py-1">
                {navLinks.map((navLink) => (
                  <li key={navLink.url}>
                    <a
                      href={navLink.url}
                      className={`text-sm md:text-lg flex flex-col items-center w-[4.5rem] md:w-auto dark:fill-textlight md:mr-6 md:hover:text-brandPrimary md:dark:hover:text-brandAccentLight link-outline ${
                        currentSection === navLink.text.toLocaleLowerCase() &&
                        'text-brandPrimary dark:text-brandAccentLight fill-brandPrimary dark:fill-brandAccentLight'
                      }`}
                    >
            ...
            >
              <svg
                ...
                width="24"
                height="24"
                className="fill-textlight hidden dark:inline-block transform scale-110 md:dark:hover:fill-brandAccentLight"
              >
              ...
              <svg
                ...
                width="24"
                height="24"
                className="dark:hidden transform scale-90 md:hover:fill-brandPrimary "
              >
